...
async function runStockCheck(client: AmulClient): Promise<void> {
  ...
  for (const recipient of recipients) {
    for (const [index, message] of messages.entries()) {
      console.log(
        `[App] Sending message ${index + 1}/${messages.length} to ${recipient}`
      )
  ...
  console.log(
    `[App] Sent ${messages.length} message(s) to ${recipients.length} recipient(s). Available products: ${targetProducts.length}`
  )
}
...
async function main(): Promise<void> {
  ...
  try {
    await runStockCheck(client)
  } catch (error) {
    console.error('[App] Stock check failed', error)
    process.exitCode = 1
    return
  }

  console.log('[App] Stock check finished; exiting')
}
